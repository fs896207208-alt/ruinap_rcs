...
public class ConsoleCommand {
     ...
     * @param messages 信息数组
     */
    public String getPoint(String[] messages) {
        String message = null;
        if (messages.length != 2) {
        ...
        // 获取点位信息
        RcsPoint rcsPoint = mapManager.getPointByAlias(messages[1]);
        if (rcsPoint == null) {
...
//        }
     ...
     * @param messages 信息数组
     */
    public String agvReLocation(String id, String[] messages) {
        String result = null;
        if (messages.length != 4) {
            StringBuilder builder = StrUtil.builder();
            builder.append("输入的格式不正确，正确指令为(括号里的参数)：\n");
     ...
     */
    public boolean sendReLocationCommand(RcsAgv rcsAgv, Integer mapId, Integer pointId, int angle) {
        boolean flag = false;
        ...
        if (socketOnline) {
            ...
            try {
                ...
                if (JSONUtil.isTypeJSON(resultStr)) {
                    ...
                    RcsLog.consoleLog.error("{} 重定位AGV返回的指令数据不格式不正确：{}", agvId, resultStr);
                }
            } catch (Exception e) {
                e.printStackTrace();
                RcsLog.consoleLog.error("{} 重定位AGV返回的指令异常：{}", agvId, e.getMessage());
            }
        }
     ...
     * @param messages 信息数组
     */
    public String cancelAGVTask(String id, String[] messages) {
        String result = null;
        if (messages.length != 2) {
...
//     * @param messages 信息数组
//     */
//    public String pauseTask(String id, String[] messages) {
//        if (messages.length < 2) {
...
//            // 获取AGV任务信息
//            AgvTask agvTask = agvManager.getAgvTaskCache().getOrDefault(agvId, null);
//            if (agvTask != null) {
//                RcsPoint currentPlanOrigin = mapManager.getRcsPoint(agv.getMapId(), agvTask.getTaskStartId());
//                RcsPoint currentPlanDestin = mapManager.getRcsPoint(agv.getMapId(), agvTask.getTaskEndId());
//
...
//     * @param messages 信息数组
//     */
//    public String recoverTask(String id, String[] messages) {
//        if (messages.length < 2) {
//            String message = "输入的格式不正确，正确指令为(括号里的参数)：\n" +
//                    "[6] [空格] [AGV编号]";
//            NettyServer.getServer(ProtocolEnum.WEBSOCKET_SERVER).sendMessage(id, message);
...
//        }
//        //获取AGV
//        RcsAgv rcsAgv = agvManager.getRcsAgvByCode(messages[1]);
//        if (rcsAgv == null) {
//            String message = "AGV[" + messages[1] + "]不存在或离线";
//            NettyServer.getServer(ProtocolEnum.WEBSOCKET_SERVER).sendMessage(id, message);
//            return message;
...
//     * @param messages 信息数组
//     */
//    public String operateSimulation(String serverId, String[] messages) {
//        if (messages.length < 4) {
//            StringBuilder builder = StrUtil.builder();
//            builder.append("输入的格式不正确，正确指令为(括号里的参数)：\n");
//            builder.append("==================== 其  他 ====================\n");
...
//            case 0:
//                //获取AGV数据
//                RcsAgv rcsAgv = agvManager.getRcsAgvByCode(messages[2]);
//                if (rcsAgv == null) {
...
//     * @return 状态 false:失败 true:成功
//     */
//    public boolean sendMoveResetCommand(RcsAgv rcsAgv, Map<String, String> agvLink, int pointId, int yaw, int battery, int load) {
//        boolean flag = false;
//        String agvId = rcsAgv.getAgvId();
//
//        //获取AGV当前点位
...
//                    }
//                } else {
//                    RcsLog.consoleLog.error("{} 重置仿真AGV返回的指令数据不格式不正确：{}", agvId, resultStr);
//                }
//            } catch (Exception e) {
//                e.printStackTrace();
//                RcsLog.consoleLog.error("{} 重置仿真AGV返回的指令异常：{}", agvId, e.getMessage());
//            }
//        }
//
//        return flag;
