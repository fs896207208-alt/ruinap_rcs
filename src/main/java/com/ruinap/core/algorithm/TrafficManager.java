...
public class TrafficManager {
    ...
    /**
     * 路径统一剪枝与路径审查 (Prune & Review - 包含业务规则拦截)
     * <p>
     * 1. 剪枝 (Pruning): 剔除历史节点。
     * 2. 占用回退 (Retreat): 遇阻时向后倒退指定点数 (RETREAT_POINT)。
     * 3. 路口限流 (Intersection): 限制一次性下发的路口数量 (STOP_INTERSECTION)。
     * 4. 短距拦截 (Threshold): 不满足最小下发长度则阻断下发 (PLAN_ALLOW_DELIVERY_THRESHOLD)。
     *
     * @param agvId        当前申请路线的 AGV 编号
     * @param currentPoint AGV 当前实际所在的物理点位
     * @param expectRoutes 完整预期路径
     * @return 经过审查后的安全路径段
     */
    public List<RcsPoint> pruneAndReviewPath(String agvId, RcsPoint currentPoint, List<RcsPoint> expectRoutes) {
        if (expectRoutes == null || expectRoutes.isEmpty()) {
            return new ArrayList<>();
        }

        // 1. 读取业务路径控制参数
        int retreatPoint = coreYaml.getAlgorithmCommon().getOrDefault("retreat_point", 1);
        int stopIntersection = coreYaml.getAlgorithmCommon().getOrDefault("stop_intersection", 1);
        int planAllowDeliveryThreshold = coreYaml.getAlgorithmCommon().getOrDefault("plan_allow_delivery_threshold", 1);

        // ==========================================
        // 步骤 A: 内存级剪枝 (确定起步安全点)
        // ==========================================
        List<RcsPoint> prunedPath = new ArrayList<>();
        int currentIndex = expectRoutes.indexOf(currentPoint);
        if (currentIndex != -1) {
            prunedPath.addAll(expectRoutes.subList(currentIndex, expectRoutes.size()));
        } else {
            prunedPath.addAll(expectRoutes);
        }

        // ==========================================
        // 步骤 B: 路径审查与业务限制过滤
        // ==========================================
        List<RcsPoint> safePath = new ArrayList<>();
        int intersectionCount = 0;

        for (int i = 0; i < prunedPath.size(); i++) {
            RcsPoint targetPoint = prunedPath.get(i);

            // 1. 自身当前所在的点位必定安全，直接放行
            if (targetPoint.equals(currentPoint)) {
                safePath.add(targetPoint);
            } else {
                // 2. O(1) 极速锁校验：前方点位是否被其他车辆占用
                RcsPointOccupy occupy = mapManager.getPointOccupy(targetPoint.getMapId(), targetPoint.getId());

                if (occupy != null && occupy.isPhysicalBlocked() && !occupy.getDeviceOccupyState(agvId)) {
                    // 【核心业务 1：遇阻回退】
                    if (safePath.isEmpty()) {
                        // 隐患排雷：如果出现极端的漂移且前方第一步就被占，防止 subList 发生越界异常，强行保底当前点
                        safePath.add(currentPoint);
                    } else {
                        // 动态计算回退点数，保底不低于 1 个点
                        int keepCount = Math.max(1, safePath.size() - retreatPoint + 1);
                        // 防御越界
                        keepCount = Math.min(keepCount, safePath.size());
                        safePath = new ArrayList<>(safePath.subList(0, keepCount));
                    }
                    RcsLog.algorithmLog.info("{} 交通审查拦截: 前方节点 [{}] 被占据，已触发回退下发截断。", agvId, targetPoint.getName());
                    break;
                }
            ...
            // 3. 【核心业务 2：真实十字路口/岔路口限流】
            List<RcsPointTarget> outgoingEdges = mapManager.getOutgoingEdges(targetPoint.getMapId(), targetPoint.getId());
            if (outgoingEdges != null) {
                int forwardChoices = 0;
                // 获取 AGV 走到 targetPoint 的“上一个物理点位”，用于排除回头路
                // 如果是当前起点 (i==0)，上一个点通常算作无（当前正在原地，还没发生空间位移）
                RcsPoint prevPoint = (i > 0) ? prunedPath.get(i - 1) : null;

                for (RcsPointTarget edge : outgoingEdges) {
                    // 如果这条出边是指向“来时的路”，直接忽略（AGV 正常不会倒车行驶）
                    if (prevPoint != null && edge.getId() == prevPoint.getId() && Objects.equals(targetPoint.getMapId(), prevPoint.getMapId())) {
                        continue;
                    }
                    forwardChoices++;
                }

                // 核心判定：如果刨除来时的路，前方依然有超过 1 条分支，说明遇到了真实的岔路/十字路！
                if (forwardChoices > 1) {
                    if (intersectionCount >= stopIntersection) {
                        RcsLog.algorithmLog.info("{} 交通审查拦截: 达到路口数限制 [{}], 已在真实的岔路口前截断路径。", agvId, stopIntersection);
                        break;
                    }
                    intersectionCount++;
                }
        ...
        // ==========================================
        // 步骤 C: 下发阈值防御 (由方法内部完美兜底！)
        // ==========================================
        if ((safePath.size() - 1) < planAllowDeliveryThreshold) {
            RcsLog.algorithmLog.warn("{} 交通审查: 审查后的可用步数 [{}] 小于阈值 [{}]，下发原地驻留指令",
                    agvId, safePath.size() - 1, planAllowDeliveryThreshold);

            // 【用户绝杀建议落地】：不再返回空集合！
            // 返回包含当前起点元素的集合 (size=1)，这在 VDA5050 协议中代表“原地驻留” (Stay Put)。
            // 这样既拦截了危险的前进，又防止了上游 A* 恐慌性重算！
            List<RcsPoint> holdPositionPath = new ArrayList<>();
            holdPositionPath.add(!safePath.isEmpty() ? safePath.getFirst() : currentPoint);
            return holdPositionPath;
        }
