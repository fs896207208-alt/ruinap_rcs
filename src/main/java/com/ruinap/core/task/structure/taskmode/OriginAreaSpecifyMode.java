 ...
 *
 * @author qianye
 * @create 2025-03-11 11:11
 */
@Component
public class OriginAreaSpecifyMode implements TaskModeHandle {
    @Autowired
    ...
    public RcsAgv handle(RcsTask rcsTask) {
        ...
        if (originPoint == null) {
            ...
            // 添加告警
            alarmManager.triggerAlarm("0", rcsTask.getTaskGroup(), rcsTask.getTaskCode(), AlarmCodeEnum.E11001, rcsTask.getOrigin(), "rcs");
            return null;
        }
        ...
        //获取任务起点区域指定AGV
        Map<String, String> taskOriginSpecify = taskCommon.getTaskOriginArea();
        ...
        //根据点位区域获取相关AGV的字符串表示
        String agvStr = taskOriginSpecify.get(originPoint.getAreaCode());
