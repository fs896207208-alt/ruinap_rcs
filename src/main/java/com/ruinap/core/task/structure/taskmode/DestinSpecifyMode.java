 ...
 *
 * @author qianye
 * @create 2025-03-11 11:10
 */
@Component
public class DestinSpecifyMode implements TaskModeHandle {
    @Autowired
    ...
    public RcsAgv handle(RcsTask rcsTask) {
        ...
        // 获取任务终点
        RcsPoint destinPoint = mapManager.getPointByAlias(rcsTask.getDestin());
        if (destinPoint == null) {
            // 如果获取终点失败，记录错误日志并触发告警
            RcsLog.algorithmLog.error("{} 任务终点[{}]获取不到点位数据", rcsTask.getTaskCode(), rcsTask.getDestin());
            // 添加告警
            alarmManage.triggerAlarm("0", rcsTask.getTaskGroup(), rcsTask.getTaskCode(), AlarmCodeEnum.E11001, rcsTask.getDestin(), "rcs");
            return null;
        }
        ...
        //获取任务终点指定AGV
        Map<String, String> taskDestinSpecify = taskCommon.getTaskDestinSpecify();
        if (taskDestinSpecify == null) {
            // 如果获取任务终点指定的AGV配置失败，记录错误日志并返回null
            RcsLog.algorithmLog.error("{} 配置文件获取不到任务终点指定AGV配置", rcsTask.getTaskCode());
            return null;
        }

        //根据任务终点获取相关AGV的字符串表示
        String agvStr = taskDestinSpecify.get(destinPoint.getMapId() + "-" + destinPoint.getId());
