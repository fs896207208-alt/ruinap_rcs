package com.ruinap.adapter.communicate.commevent;
 ...
 */
@Component
public class AgvWebSocketClientEvent extends AbstractClientEvent<TextWebSocketFrame> {
    @Autowired
    ...
    public void receiveMessage(ClientAttribute attribute, TextWebSocketFrame frame) {
        ...
        if (frame != null) {
            ...
            if (typeJsonObj) {
                ...
                //记录日志
                RcsLog.communicateLog.info(RcsLog.getTemplate(5), RcsLog.randomInt(), equipmentType + "_" + clientId, "rec_data", jsonObject.getInt("DataStamps"), jsonObject);
                //事件分支
     ...
     * @param body 数据
     */
    private void dispatchAgvState(String pact, String equipmentType, String clientId, JSONObject body) {
        ...
        agvManager.getAgvTaskCache().compute(clientId, (key, oldTask) -> {
...
//            agvTask.setPathState(pathState);
     ...
     * @param body 数据
     */
    private void agvState(String pact, String equipmentType, String clientId, JSONObject body) {
     ...
     * @param body 数据
     */
    private void agvMap(String pact, String equipmentType, String clientId, JSONObject body) {
        //获取地图检查数据
        ...
        if (agvId == null || agvId.isEmpty() || mapId == null || mapId <= 0 || mapCheck == null || mapCheck.isEmpty()) {
            //AGV传入的数据有误
            RcsLog.consoleLog.error("{} AGV传入的地图数据错误，JSON数据: {}", agvId, body);
            return;
        }

        //获取地图MD5数据
        String rcsMapMd5 = mapManager.getSnapshot().getMd5(mapId);
        if (!mapCheck.equalsIgnoreCase(rcsMapMd5)) {
            //地图不一样则告警
            RcsLog.consoleLog.error("{} AGV地图数据和调度系统不一致，请检查地图数据", agvId);
            //添加告警
            alarmManager.triggerAlarm(agvId, AlarmCodeEnum.E10001, "rcs");
            RcsAgv rcsAgv = agvManager.getRcsAgvByCode(agvId);
            rcsAgv.setMapChecked(false);
        } else {
            RcsAgv rcsAgv = agvManager.getRcsAgvByCode(agvId);
            rcsAgv.setMapChecked(true);
            RcsLog.consoleLog.info("{} AGV地图数据校验通过", agvId);
        }
    }
     ...
     * @param body 数据
     */
    private void connectSuccess(String pact, String equipmentType, String clientId, JSONObject body) {
        //获取地图检查数据
        String agvId = body.getStr("AgvId");
        Integer mapId = body.getInt("MapId");
        if (mapId == null) {
